...
// Ada tambahan function untuk cek kromosom yang kembar
...
int getValue(vector<int> data) {
    ...
    for (int i = 0; i < data.size() - 1; i++) {
        for (int j = i + 1; j < data.size(); j++) {
            if (data[i] == data[j] || abs(data[j] - data[i]) == abs(j - i)) {
                count++;
            }
        }
    }
...
void deleteTwin() {
    for (int i = 0; i < population.size() - 1; i++) {
        for (int j = i + 1; j < population.size(); j++) {
            if (compare(population[i], population[j])) {
...
void eliminate() {
    deleteTwin();
    cout << (min_data + 3.0 * (max_data - min_data) / 4) / max_value * 100 << endl;
    ...
    for (int i = 0; i < population.size(); i++) {
        ...
        if (chance < prob) {
            ...
            if (population[i][n] >= max_data) {
                vector<int> data = population[i];
                mutate_cromosome(data);
                population.push_back(data);
            }
            else {
